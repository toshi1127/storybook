...
describe('layout API', () => {
  describe('setOptions', () => {
    ...
    it('should be dark theme if prefers-color-scheme is dark', () => {
      ...
      layoutApi.setOptions({ theme });

      expect(getLastSetStateArgs()[0].theme).toEqual(theme);
    });

    it('should be light theme if prefers-color-scheme is light', () => {
      window.matchMedia = jest.fn().mockImplementation(query => {
        return {
          matches: false,
          media: query,
